class FlightsController < ApplicationController
  ...
  def create
    ...
    @response = HTTParty.get("https://api.test.sabre.com/v1/shop/flights?origin=#{origin_input}&destination=#{destination_input}&departuredate=#{depart_date}&returndate=#{return_date}&onlineitinerariesonly=N&limit=10&offset=1&eticketsonly=N&sortby=totalfare&order=asc&sortby2=departuretime&order2=asc", :headers => { "Authorization" => "Bearer Shared/IDL:IceSess\/SessMgr:1\.0.IDL/Common/!ICESMS\/ACPCRTC!ICESMSLB\/CRT.LB!-3663573353839303422!35306!0!!E2E-1"})
  end
end
...
# ["{:id=>\"dateField\"}(1i)"]["{:id=>\"dateField\"}(2i)"]["{:id=>\"dateField\"}(3i)"]

# .fetch("{:id=>\"dateField\"}(1i)")




# https://api.test.sabre.com/v1/shop/flights?origin=&destination=BOS&departuredate=["2014", "5", "14"]&returndate=["2014", "5", "15"]&onlineitinerariesonly=N&limit=10&offset=1&eticketsonly=N&sortby=totalfare&order=asc&sortby2=departuretime&order2=a
